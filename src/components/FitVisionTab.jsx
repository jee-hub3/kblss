                ...
                <div className="max-w-4xl mx-auto text-center">
                    <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
                        ...
                        <motion.p
                            variants={fadeInUp}
                            custom={2}
                            className="text-sm md:text-base text-slate-600 leading-relaxed font-medium break-keep mx-auto"
                        >
                            ...
                            >
                                ...
                                <div className="w-full md:w-1/2 flex justify-center">
                                    <div className="w-full aspect-[4/3] rounded-[2rem] overflow-hidden relative group">
                                        <img src={['/image/value-action.png', '/image/value-problem.png', '/image/value-teamwork.png'][i]} alt={v.title} className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" />
                                    </div>
                                </div>
                            </motion.div>
                        ))}
                            ...
                            <div className="grid grid-cols-4 gap-6">
                                {roadmapSteps.map((step, i) => (
                                    <motion.div key={i} variants={fadeInUp} custom={i} className="flex flex-col items-center text-center">
                                        {/* Number Node */}
                                        <div className={`relative z-10 w-24 h-24 rounded-full bg-slate-50 flex items-center justify-center border-4 border-white mb-8 text-slate-900 font-extrabold text-4xl tracking-tighter shadow-sm`}>
                                            {step.number}
                                        </div>
                        ...
                        <div className="space-y-16 py-8">
                            {roadmapSteps.map((step, i) => (
                                <motion.div key={i} variants={fadeInUp} custom={i} className="relative flex flex-col gap-2 pl-12">
                                    {/* Number Node */}
                                    <div className={`absolute -left-8 top-0 w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center border-4 border-white shadow-sm text-slate-900 font-extrabold text-2xl tracking-tighter`}>
                                        {step.number}
                                    </div>
